  ...
  async getAllEnrollments(forceRefresh = false) {
    ...
    try {
      ...
      const timeoutId = setTimeout(() => {
        controller.abort('Request timeout - enrollment fetch took too long')
      ...
      if (response.ok) {
        ...
        if (data && Array.isArray(data.enrollments)) {
          console.log(`📊 Processing ${data.enrollments.length} enrollments from API`)
          data.enrollments.forEach(enrollment => {
            const courseId = enrollment.courseId?.toString() || enrollment._id?.toString()
            if (courseId) {
          ...
          console.log(`📊 Processing ${data.courses.length} course enrollments from API`)
          ...
          console.log(`📊 No enrollments found in API response`)
        }
        ...
        if (this.cache.has(cacheKey)) {
          const cached = this.cache.get(cacheKey)
          console.log(`⚠️ Using stale all enrollments cache due to API error:`, cached.size, 'enrollments')
          return { enrollments: new Map(cached), cached: true, stale: true, error: errorDetails }
        }
        
        return { enrollments: new Map(), cached: false, error: errorDetails }
      }
    } catch (error) {
      ...
      if (error.name === 'AbortError') {
        errorMessage = 'Request timeout - enrollment fetch took too long'
      } else if (error.message.includes('fetch')) {
        errorMessage = 'Network error - could not connect to server'
      }
      
      console.error(`📋 Enrollment fetch error:`, errorMessage)
      
      // Return cached data if available
      if (this.cache.has(cacheKey)) {
        const cached = this.cache.get(cacheKey)
        console.log(`⚠️ Using stale all enrollments cache due to error:`, cached.size, 'enrollments')
        return { enrollments: new Map(cached), cached: true, stale: true, error: errorMessage }
      }
      
      // Return empty map with error info
      return { enrollments: new Map(), cached: false, error: errorMessage }
    }
  }
