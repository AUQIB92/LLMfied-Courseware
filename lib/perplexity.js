...
if (!process.env.PERPLEXITY_API_KEY) {
  ...
  console.log("Please set your Perplexity API key in your environment variables:");
  console.log(
...
async function callPerplexityWithRetry(
  ...
  console.log(`📝 Perplexity prompt length: ${payload.messages[0].content.length} characters`);
  ...
  try {
    const payload = {
      ...
      messages: [
        {
          role: "user",
          content: "Search the web and tell me what the current date is. Respond with just 'Connection successful' and today's date."
        }
      ],
      max_tokens: 100,
    };

    const responseText = await callPerplexityWithRetry(payload, 2, 1000);
...
export async function generatePerplexityQuiz(moduleContent, difficulty = "medium", context = {}) {
  const {
    subject = "general",
    examType = "general",
...
- Include sources array for each question when using web-searched information
  ...
  try {
    ...
    if (parsed && parsed.questions) {
      console.log(`✅ Successfully generated ${parsed.questions.length} quiz questions with Perplexity`);
      return parsed;
    } else {
      console.warn("⚠️ Perplexity response doesn't have expected questions format");
      return { questions: [] };
    }
  } catch (error) {
...
- Prioritize high-quality, current resources from reputable sources
    ...
    max_tokens: 6000,
    search_domain_filter: ["edu", "org", "com"],
    return_citations: true,
  };
  ...
  try {
    ...
    if (parsed) {
      ...
      console.warn("⚠️ Perplexity response doesn't have expected resources format");
      return {
        books: [],
        courses: [],
        articles: [],
...
- Use web search results to enhance content with current information
  ...
  try {
    ...
    if (parsed && parsed.summary) {
      ...
      console.warn("⚠️ Perplexity response doesn't have expected module summary format");
      return {
...
export async function generatePerplexityCompetitiveExamModuleSummary(content, context = {}) {
  const {
    learnerLevel = "intermediate",
...
${content}
  ...
  try {
    console.log(`🔍 Generating current competitive exam content for ${examType} with Perplexity...`);
    const response = await fetch('https://api.perplexity.ai/chat/completions', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${process.env.PERPLEXITY_API_KEY}`,
    ...
    console.log(`✅ Perplexity competitive exam response received (${content.length} chars)`);
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        console.log(`⚠️ No JSON found in Perplexity response, using text processing`);
        // Extract data with regex if JSON parsing fails
        return extractDataWithRegex(content);
      }
    } catch (parseError) {
      console.log(`⚠️ JSON parsing failed, using text processing:`, parseError.message);
      return extractDataWithRegex(content);
    }
