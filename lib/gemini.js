...
export async function generateAcademicSubsectionSummary(content, context = {}) {
  ...
  const prompt = `
  You are an expert academic content generator. Produce ONLY HTML pages with robust LaTeX math for the topic. Do not include summaries, insights, practical applications, why-this-matters, or any learning resources. Output must be a single JSON object following the minimal schema below and nothing else.

  CRITICAL MATH HANDLING REQUIREMENTS:
  
  1️⃣ Wrap all math in \\( ... \\) or \\[ ... \\] and escape backslashes twice
  ...
  - This forces \\times → \\\\\\\\times in HTML output
  - Example: <p>A relation \\( R \\subseteq A \\times B \\) is ...</p>
  
  2️⃣ Never send math inside HTML without escaping
  ...
  3️⃣ Separate math from HTML when possible
    ...
    "detailedSubsections": [
      {
        ...
        "pages": [
          {
            "pageTitle": "Clear page title",
            "html": "<h3>Heading</h3><p>Concept explanation in HTML. Math: \\(a^2+b^2=c^2\\) and \\[\\\\int_a^b f(x)\\\\,dx\\].</p>",
