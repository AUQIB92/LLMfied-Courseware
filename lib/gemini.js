...
export async function generateCompetitiveExamModuleSummary(
  ...
  try {
    ...
    if (
      ...
      console.log(
        "✅ Successfully parsed valid competitive exam module summary"
      );
      // Apply LaTeX arrow fixes to the parsed response
      return fixLatexInObject(parsed);
    } else {
      console.warn("Parsed response doesn't have expected structure:", parsed);
    ...
    // Create a more detailed fallback based on the content
    const contentTitle = content
      ...
      : "this topic";
    const fallbackTitle =
      contentTitle.length > 0 ? contentTitle : "this module";
