...
export async function generateCompetitiveExamModuleSummary(content, context = {}) {
  ...
  const prompt = `
    ...
    - For block/display equations, use $$...$$ delimiters (e.g., $$\\sum_{i=1}^{n} i = \\frac{n(n+1)}{2}$$)
    - Use proper LaTeX notation for fractions (\\frac{numerator}{denominator}), exponents (x^{power}), subscripts (x_{index})
    - Format matrices using the matrix environment: $$\\begin{bmatrix} a & b \\\\ c & d \\end{bmatrix}$$
    - Format multi-line equations using the align environment: $$\\begin{align} x &= y \\\\ &= z \\end{align}$$
    - Format integrals, limits, and summations properly: $\\int_{a}^{b} f(x) dx$, $\\lim_{x \\to 0} f(x)$, $\\sum_{i=1}^{n} a_i$
    - Use \\text{} for text within equations: $\\text{area} = \\pi r^2$
    - Format all mathematical symbols properly (e.g., \\alpha, \\beta, \\theta, \\pi, \\infty)
    - Use proper spacing in equations: $f(x) = x^2 + 2x + 1$
    ...
    7. Detailed subsections with MULTI-PAGE EXPLANATIONS:
       ...
       CRITICAL: For each detailed subsection, you MUST create EXACTLY 8 PAGES of detailed explanation:
       
       CONCEPT-FOCUSED PAGES (Pages 1-5):
       - MANDATORY: Page 1: "Introduction & Foundation" (300-400 words) - Basic concepts, definitions, context, why this concept matters
       - MANDATORY: Page 2: "Core Theory & Principles" (300-400 words) - Deep dive into theoretical understanding and fundamental principles
       - MANDATORY: Page 3: "Essential Formulas & Derivations" (300-400 words) - All key formulas with step-by-step derivations and proofs
       - MANDATORY: Page 4: "Concept Applications & Examples" (300-400 words) - How the concept applies in various scenarios with detailed examples
       - MANDATORY: Page 5: "Conceptual Problem Solving" (300-400 words) - Systematic approach to solving problems using the concept
       
       EXAM-FOCUSED PAGES (Pages 6-8):
       - MANDATORY: Page 6: "Exam Strategies & Speed Techniques" (300-400 words) - Time-saving methods, shortcuts, and exam-specific strategies
       - MANDATORY: Page 7: "Common Traps & Error Prevention" (300-400 words) - Typical mistakes, traps in questions, and how to avoid them
       - MANDATORY: Page 8: "Mastery & Exam Readiness" (300-400 words) - Final review, memory aids, and exam preparation checklist
