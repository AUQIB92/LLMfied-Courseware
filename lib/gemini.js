...
export async function generateModuleSummary(content, context = {}) {
  ...
  const prompt = `
    ...
    STRICT MATH AND MARKDOWN OUTPUT RULES (APPLIES ANY TIME YOU RETURN CONTENT):
    - Use LaTeX delimiters for math: inline math MUST be wrapped in $ ... $, display math MUST be wrapped in $$ ... $$
    - Use single backslashes in LaTeX commands (e.g., \\frac{a}{b}, \\sqrt{x}, \\sum, \\int)
    - Do NOT escape dollar signs ($) and do NOT place math inside code blocks
    - Do NOT convert math to MathML; keep LaTeX as text between the $ delimiters
    - Use proper Markdown formatting: # ## ### for headers, **bold**, *italic*, - for lists, tables, etc.
    - Keep equations on their own lines for display math using $$ ... $$ blocks
    - Use standard Markdown syntax for all formatting instead of HTML tags
      ...
      "detailedSubsections": [
        {
          ...
          "pages": [
            {
              "pageTitle": "Clear page title",
              "content": "Well-structured Markdown text with LaTeX using $inline$ and $$display$$ math where appropriate. Use proper Markdown formatting for headers, lists, emphasis, etc.",
              "keyTakeaway": "One-sentence takeaway for this page"
            }
          ],
  ...
  // Call Gemini with retries and proper awaiting
  const responseText = await callGeminiWithRetry(model, prompt, 4, 2500);
  ...
  try {
    ...
    // Parse the JSON with extra safeguards using robust parsing libraries
    let parsed;
    try {
      // First try standard JSON.parse
      parsed = JSON.parse(jsonText);
      console.log("Successfully parsed JSON response with standard parser");
    } catch (parseError) {
      console.log("Standard JSON parse failed, trying with jsonrepair...", parseError.message);
      try {
        // Try to repair and parse the JSON
        const repairedJson = jsonrepair(jsonText);
        parsed = JSON.parse(repairedJson);
        console.log("Successfully parsed JSON response with jsonrepair");
      } catch (repairError) {
        console.log("jsonrepair failed, trying with JSON5...", repairError.message);
        try {
          // Try with JSON5 parser (more lenient)
          parsed = JSON5.parse(jsonText);
          console.log("Successfully parsed JSON response with JSON5");
        } catch (json5Error) {
...
export async function generateAcademicSubsectionSummary(content, context = {}) {
  ...
  const prompt = `
  You are an expert academic content generator. Produce ONLY HTML content with robust LaTeX math for the topic. Do not include summaries, insights, practical applications sections, or external learning resources. Output must be a single JSON object following the minimal schema below and nothing else.

  CRITICAL: NEVER use math placeholders like **MATH*INLINE*0** or **MATH*BLOCK*1**. Always use proper LaTeX delimiters like $...$ and $$...$$ directly in the HTML.

  MANDATORY STRUCTURE PER SUBSECTION
  - Produce at least 5 distinct pages under each subsection.
  - Each page must contain:
    • A clear heading (h3) and explanatory paragraphs targeted at learners.
    • At least one worked example with step-by-step reasoning.
    • "Important Results" (key theorems/identities) where relevant.
    • At least one formal proof somewhere within the subsection (can be placed on one or more pages). Label proof sections clearly with a heading such as "Proof" and present the math using proper LaTeX.
  - Suggested page outline (flexible):
    1) Concept Introduction and Intuition
    2) Core Definitions and Properties
    3) Worked Examples (basic to intermediate)
    4) Important Results and Proofs (at least one full proof)
    5) Advanced Example or Edge Cases

  CRITICAL LATEX MATH HANDLING REQUIREMENTS
  1) Use standard LaTeX delimiters in HTML content:
     - Use $...$ for inline math
     - Use $$...$$ for display math (on separate lines)
     - Always wrap mathematical expressions properly
     - Example: A relation $R \\subseteq A \\times B$ is a subset of the Cartesian product.

  2) JSON Escaping Rules for LaTeX
     - In JSON strings, each backslash must be doubled: \\ becomes \\\\
     - For LaTeX commands in JSON: \\frac becomes \\\\frac
     - For Greek letters in JSON: \\alpha becomes \\\\alpha
     - NEVER use placeholders like **MATH*INLINE*0** - always use proper LaTeX delimiters
     - Example: "html": "<p>The formula $\\\\frac{a}{b}$ represents a fraction.</p>"
     - WRONG: "html": "<p>The formula **MATH*INLINE*0** represents a fraction.</p>"
     - RIGHT: "html": "<p>The formula $\\\\frac{a}{b}$ represents a fraction.</p>"

  3) Complete LaTeX Syntax Requirements in HTML
     ...
     - Subscripts/Superscripts: $x_{i}^{2}$ for indexed variables
     - Sets: $A \\\\subseteq B$, $x \\\\in S$, $\\\\emptyset$, $\\\\cup$, $\\\\cap$
     - Logic: $\\\\forall, \\\\exists, \\\\implies, \\\\iff$
     - Matrices: $\\\\begin{pmatrix} a & b \\\\\\\\ c & d \\\\end{pmatrix}$

  4) Display Math Examples
  ...
  5) Math Content Requirements
     - Include ALL relevant mathematical formulas and equations for the topic
     - Provide step-by-step derivations with proper LaTeX formatting
     - Show mathematical proofs with clear logical steps
     - Include numerical examples with calculations
     - Use mathematical notation consistently throughout

  HTML FORMATTING RULES
  - Use proper HTML tags: <h1>, <h2>, <h3> for headers, <strong> for bold, <em> for italic, <ul>/<li> for lists, etc.
  - Use standard HTML table syntax
    ...
    "detailedSubsections": [
      {
        ...
        "pages": [
          {
            "pageTitle": "Clear page title",
            "html": "<h3>Fundamental Theorem</h3>\\n<p>The Pythagorean theorem states that $a^2 + b^2 = c^2$ for right triangles.</p>\\n<h3>Worked Example</h3>\\n<p>Consider a triangle with sides $a = 3$ and $b = 4$. Then $c = \\\\sqrt{3^2 + 4^2} = \\\\sqrt{9 + 16} = \\\\sqrt{25} = 5$.</p>\\n<h3>Proof</h3>\\n<p>The proof follows from geometric considerations and leads to the integral:</p>\\n$$\\\\int_0^{\\\\infty} e^{-x^2} dx = \\\\frac{\\\\sqrt{\\\\pi}}{2}$$",
            "math": ["a^2 + b^2 = c^2", "c = \\\\sqrt{a^2 + b^2}", "\\\\int_0^{\\\\infty} e^{-x^2} dx = \\\\frac{\\\\sqrt{\\\\pi}}{2}"]
          }
        ]
      }
    ]
  ...
  INPUT CONTENT (for grounding): ${content}
  `;

  const responseText = await callGeminiWithRetry(model, prompt, 3, 2500, 45000); // 45 second timeout
...
// Generate academic learning resources for modules
export async function generateAcademicResources(moduleContent, context = {}) {
  const {
  ...
  const prompt = `
  You are an academic resource curator specializing in finding high-quality learning materials. Generate comprehensive learning resources for the given academic module content.
  ...
  For each resource type, provide 3-5 high-quality, relevant resources:
  
  1. BOOKS: Academic textbooks, reference materials, supplementary readings
  2. ARTICLES: Scholarly articles, research papers, journal publications  
  3. VIDEOS: Educational videos, lectures, documentaries from reputable sources
  4. COURSES: Online courses, MOOCs, structured learning programs
  5. TOOLS: Software, calculators, simulators, interactive tools
  6. WEBSITES: Educational websites, databases, reference sites
  7. EXERCISES: Practice problems, worksheets, interactive exercises

  QUALITY STANDARDS:
  - All resources must be academically appropriate for ${academicLevel} level
  - Prioritize reputable sources (universities, established publishers, known educators)
  - Include real, specific resources with actual titles and descriptions
  - Provide working URLs when possible, or searchable titles
  - Focus on resources that complement and enhance the module content
...

  {
    "books": [
      {
        ...
        "author": "Author name",
        "description": "Brief description of relevance and content",
        "url": "Direct link or search URL"
      }
    ],
    "articles": [
      {
        "title": "Article title",
        "author": "Author name", 
        "description": "Brief description and relevance",
        "url": "DOI link or journal URL"
      }
    ],
    "videos": [
      {
    ...
    "courses": [
      {
        "title": "Course title",
        "platform": "Platform name (Coursera, edX, etc.)",
        "description": "Course description and relevance", 
        "url": "Course URL"
      }
    ],
    "tools": [
      {
        "title": "Tool name",
        "creator": "Developer or organization",
        "description": "Tool purpose and functionality",
        "url": "Tool URL or download link"
      }
    ],
    "websites": [
      {
        "title": "Website name",
        "creator": "Organization or maintainer",
        "description": "Website content and usefulness",
        "url": "Website URL"
      }
    ],
    "exercises": [
      {
        "title": "Exercise set name",
        "creator": "Source or author",
        "description": "Type and scope of exercises",
        "url": "Exercise URL or source"
      }
    ]
  }
  ...
  const responseText = await callGeminiWithRetry(model, prompt, 3, 2000, 30000);

  try {
    console.log("Processing resource generation response...");
    ...
    const expectedTypes = ['books', 'articles', 'videos', 'courses', 'tools', 'websites', 'exercises'];
    const validatedResources = {};
    
    expectedTypes.forEach(type => {
      if (resources[type] && Array.isArray(resources[type])) {
        validatedResources[type] = resources[type]
          ...
          .map(resource => ({
            ...
            description: resource.description || "Resource description not available",
            url: resource.url || `https://www.google.com/search?q=${encodeURIComponent(resource.title || '')}`
          }));
      } else {
        validatedResources[type] = [];
      }
    });

    console.log(`✅ Successfully generated academic resources:`, {
      books: validatedResources.books.length,
      articles: validatedResources.articles.length, 
      videos: validatedResources.videos.length,
      courses: validatedResources.courses.length,
      tools: validatedResources.tools.length,
      websites: validatedResources.websites.length,
      exercises: validatedResources.exercises.length,
      total: Object.values(validatedResources).reduce((sum, arr) => sum + arr.length, 0)
    });

    return validatedResources;

  } catch (parseError) {
    console.error("Failed to parse resource generation response:", parseError);
    console.log("Raw response:", responseText);
    
    // Return empty resource structure as fallback
    return {
      books: [],
      articles: [],
      videos: [],
