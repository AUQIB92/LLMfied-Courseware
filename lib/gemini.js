...
export async function generateModuleSummary(content, context = {}) {
  ...
  const prompt = `
    ...
    STRICT MATH AND HTML OUTPUT RULES (APPLIES ANY TIME YOU RETURN HTML):
    ...
    - Do NOT escape dollar signs ($) and do NOT place math inside <code> or fenced code blocks
    - Do NOT convert math to MathML; keep LaTeX as text between the $ delimiters
    - Only use safe, semantic HTML tags: <h2>, <h3>, <p>, <ul>, <ol>, <li>, <table>, <thead>, <tbody>, <tr>, <th>, <td>, <pre>, <code>, <blockquote>, <hr>, <img>
    - Do NOT include <script>, <style>, or inline event handlers; minimal inline styles only if absolutely necessary
    - Keep equations on their own lines inside paragraphs or separate <div>/<p> blocks for readability; do not wrap with extra custom classes
          ...
          "pages": [
            {
              ...
              "content": "Markdown-friendly text with LaTeX using $inline$ and $$display$$ where appropriate",
              "keyTakeaway": "One-sentence takeaway for this page",
              "html": "Well-structured, sanitized HTML equivalent of 'content' using only semantic tags; keep LaTeX delimiters ($, $$) untouched inside the HTML so a client renderer can typeset math. Do not include <script> or <style> tags."
            }
          ],
...
export async function generateAcademicSubsectionSummary(content, context = {}) {
  ...
  const prompt = `
  You are an expert academic content generator. Produce ONLY HTML pages with robust LaTeX math for the topic. Do not include summaries, insights, practical applications sections, or external learning resources. Output must be a single JSON object following the minimal schema below and nothing else.
  ...
  CRITICAL MATH HANDLING REQUIREMENTS
  1) Wrap all math in \\( ... \\) or \\[ ... \\] and escape backslashes twice
     - Return HTML where all mathematical expressions are wrapped in \\(...\\) for inline or \\[...\\] for display
     - Every LaTeX backslash must be doubled (\\\\)
     - Example: <p>A relation \\( R \\subseteq A \\times B \\) is ...</p>

  2) Never send math inside HTML without escaping
     - If sending HTML via JSON, escape it properly
     - One backslash (\\) is eaten by JSON, so you must double it (\\\\) for it to survive
     - Example: "html": "<p>A relation $R \\\\\\\\subseteq A \\\\\\\\times B$ is ...</p>"

  3) Separate math from HTML when possible
     - Return structured output with separate "html" and "math" fields
     - Example: { "html": "<p>A relation is a set of ordered pairs.</p>", "math": "R \\\\\\\\subseteq A \\\\\\\\times B" }
  ...
  HTML RULES
  - Use only semantic, safe tags: h2, h3, p, ul, ol, li, table, thead, tbody, tr, th, td, pre, code, blockquote, hr, img.
    ...
    "detailedSubsections": [
      {
        ...
        "pages": [
          {
            "pageTitle": "Clear page title",
            "html": "<h3>Heading</h3><p>Concept explanation in HTML. Math: \\(a^2+b^2=c^2\\) and \\[\\\\int_a^b f(x)\\\\,dx\\].</p><h3>Worked Example</h3><p>...steps...</p><h3>Important Results</h3><p>...</p>",
            "math": ["a^2+b^2=c^2", "\\\\int_a^b f(x)\\\\,dx"]
          }
        ]
      }
    ]
