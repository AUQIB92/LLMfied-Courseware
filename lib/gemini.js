...
function repairJsonString(jsonStr) {
  ...
  // 2. Fix unescaped quotes in strings (more robust approach)
  ...
  // 4. Fix common LLM response patterns
  repaired = repaired.replace(/```json\s*/g, ''); // Remove markdown code blocks
  repaired = repaired.replace(/```\s*$/g, ''); // Remove closing markdown
  repaired = repaired.replace(/^[^{]*/, ''); // Remove text before first {
  repaired = repaired.replace(/[^}]*$/, '}'); // Remove text after last }
  
  // 5. Fix malformed arrays and objects
  repaired = repaired.replace(/,(\s*,)+/g, ','); // Remove multiple consecutive commas
  repaired = repaired.replace(/:\s*,/g, ': null,'); // Fix empty values
  
  // 6. Fix truncated strings by closing them
  const openQuotes = (repaired.match(/(?<!\\)"/g) || []).length;
  if (openQuotes % 2 !== 0) {
    repaired += '"';
  }
  
  // 7. Balance brackets and braces more intelligently
  let braceCount = 0;
  let bracketCount = 0;
  let inString = false;
  let escaped = false;
  ...
  // 8. Final cleanup
  repaired = repaired.replace(/,(\s*[}\]])/g, '$1'); // Remove trailing commas again after balancing
...
function parseJsonProgressive(jsonStr) {
  ...
  // Much more aggressive progressive parsing with smaller increments
  const percentages = [0.95, 0.9, 0.85, 0.8, 0.75, 0.7, 0.65, 0.6, 0.55, 0.5, 0.45, 0.4, 0.35, 0.3, 0.25, 0.2];
  const lengths = percentages.map(p => Math.floor(jsonStr.length * p));
  
  for (const length of lengths) {
    try {
      let truncated = jsonStr.substring(0, length);
      
      // Find the last complete section by looking for complete object/array closures
      truncated = findLastCompleteSection(truncated);
      
      // Repair and try to parse
      const repaired = repairJsonString(truncated);
      const parsed = JSON.parse(repaired);
      
      console.log(`Successfully parsed JSON at ${length} characters (${Math.round(length/jsonStr.length*100)}% of original)`);
      return parsed;
    } catch (error) {
      console.log(`Failed to parse at ${length} characters (${Math.round(length/jsonStr.length*100)}%), trying shorter...`);
      continue;
    }
  }
  
  // Final attempt with just the first part of the JSON (summary and basic fields)
  try {
    ...
    console.log("Minimal JSON extraction also failed");
  }
...
function findLastCompleteSection(jsonStr) {
  // Look for the last complete property by finding balanced braces/brackets
  let braceCount = 0;
  let bracketCount = 0;
  let inString = false;
  let escaped = false;
