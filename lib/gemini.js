...
export async function generateCompetitiveExamModuleSummary(
  ...
  const model = genAI.getGenerativeModel({ model: "gemini-2.0-flash" });
  const prompt = `
Analyze the following educational content and generate a structured, exam-oriented learning breakdown optimized for competitive exams such as SSC, UPSC, CAT, Bank PO, GRE, etc., specifically in the domain of **${subject}** for **${examType}**.

LEARNER PROFILE:
- Target Level: **${learnerLevel}**
- Module Index: ${moduleIndex} of ${totalModules}
- Content Focus: ${content}

CUSTOMIZED GUIDELINES FOR ${learnerLevel.toUpperCase()} LEARNERS:
${
  learnerLevel === "beginner"
    ? `
- Use simple language and intuitive analogies
- Emphasize confidence-building with foundational clarity
- Include shortcuts, tips, and interactive examples for understanding
`
    : learnerLevel === "intermediate"
    ? `
- Bridge theory with practical problem-solving for exams
- Focus on solving moderate difficulty problems with speed techniques
- Include common traps and intermediate-level strategic insights
`
    : learnerLevel === "advanced"
    ? `
- Prioritize high-difficulty concepts and fast-solving strategies
- Include tough examples, optimization tips, and pattern recognition
- Focus on elimination techniques and exam mastery
`
    : `
- Provide expert-level exam-solving strategies
- Include edge-case tricks and advanced analytics
- Equip learners to mentor or teach others
`
}

MANDATORY STRUCTURE FOR EACH SUBSECTION:
Each subsection will generate exactly 12 structured pages:
1. **Page 1: Introduction & Conceptual Foundation**
2. **Page 2: Theoretical Background (Part 1)**
3. **Page 3: Theoretical Background (Part 2)**
4. **Page 4: Theoretical Background (Part 3)**
5. **Page 5: Detailed Explanation with Examples**
6. **Page 6: Real-World Applications**
7. **Page 7: Basic Problem-Solving Techniques**
8. **Page 8: Advanced Problem-Solving Techniques**
9. **Page 9: Important Formulas & Memory Tricks** (in LaTeX, with derivations and mnemonics)
10. **Page 10: Common Pitfalls, Revision Tips & Final Insights**
11. **Page 11: Short Tricks & Speed Techniques**
12. **Page 12: Key Takeaways & Exam Readiness**

🔢 PAGE REQUIREMENTS:
- Each page must contain **300–400 words** of dense, exam-relevant content.
- Use LaTeX format for math (inline: \`$\`, block: \`$$\`) with **double backslashes** (e.g., \\\\frac, \\\\sqrt).
- Every page must end with a **key takeaway**.
- Include real competitive exam relevance, tricks, and traps.

🧠 ADDITIONAL OUTPUT REQUIREMENTS:
1. Concise summary (2–3 sentences)
2. 4–5 learning objectives
3. 3–5 exam-aligned examples
4. Visual/interactive component suggestions
5. Resources section:
   - Books (with author, year, URL)
   - Courses (title, platform, URL, exam relevance)
   - Videos (title, channel, URL or search query, description)
   - Articles, Tools, Websites, and Practice Exercises (with relevance and links)

🧩 OUTPUT FORMAT:
Wrap everything in a valid JSON block inside a Markdown code block. Structure the response as follows:
        ...
        "pages": [
          {
            ...
            "mathematicalContent": [
              {
                "type": "formula|definition|example",
                "title": "Title",
                "content": "Math in LaTeX",
          ...
          ...
          {
            ...
            "content": "Summary and checklist for exam",
            "keyTakeaway": "..."
          }
        ],
        "practicalExample": "Real ${examType} problem or scenario",
        "commonPitfalls": ["...", "..."],
        "difficulty": "${learnerLevel}",
        "estimatedTime": "Time to master",
        "examRelevance": "Why this is important for ${examType}"
      }
    ]
  }
}
...
- All links must be complete and start with \`https://\`
- Include fallback \`searchQuery\` if video URLs are not exact

BEGIN ANALYSIS BASED ON THIS CONTENT:
${content}
`;
  ...
  try {
    ...
    if (!codeBlockMatch) {
      console.log(
        "No JSON code block found, trying to extract JSON directly..."
      );
    ...
    if (!parsed) {
      console.log(
        "All JSON parsing strategies failed, using regex extraction..."
      );
      try {
        parsed = extractDataWithRegex(responseText);
      } catch (regexError) {
        console.log(
          "Regex extraction also failed, creating competitive exam fallback structure..."
        );
        ...
        parsed = {
          ...
          beautifulSummaryElements: {
            keyInsights: [
              `Essential ${subject} concepts for ${examType}`,
              "Speed-solving strategies",
              "Common exam traps",
            ],
            practicalApplications: [
              `${examType} exam questions`,
    ...
    if (parsed && parsed.resources) {
      console.log(
        "Processing competitive exam resources with comprehensive URL validation..."
      );
    }
    ...
    const generateOptimizedSearchQueries = async (resources, type) => {
      try {
        ...
        const resourcesNeedingOptimization = resources.filter((resource) => {
          ...
          return (
            title && author && (!resource.url || resource.url.trim() === "")
          );
        });
        ...
        console.log(
          `🤖 Batching ${resourcesNeedingOptimization.length} ${type} resources for competitive exam search optimization`
        );

        const model = genAI.getGenerativeModel({ model: "gemini-2.0-flash" });

        const resourceList = resourcesNeedingOptimization
          .map(
            (resource, index) =>
...
${
  type === "books"
    ? `- "${subject} ${examType} book author name exam preparation"`
    : ""
}
${
  type === "courses"
    ? `- "${examType} ${subject} course Unacademy BYJU'S online preparation"`
    : ""
}
${
  type === "videos"
    ? `- "${examType} ${subject} tutorial video channel exam preparation"`
    : ""
}
${
  type === "articles"
    ? `- "${examType} ${subject} article exam strategy tips"`
    : ""
}
${
  type === "tools"
    ? `- "${examType} exam preparation tool mock test software"`
    : ""
}
`;
        ...
        try {
          ...
          console.log(
            `✅ Generated ${
              Object.keys(optimizedQueries).length
            } optimized search queries for ${type}`
          );
          return optimizedQueries;
        } catch (parseError) {
          console.log(
            `⚠️ Failed to parse batched optimization response, using fallback`
          );
          return {};
        }
      } catch (error) {
        console.log(
          `⚠️ Batched LLM optimization failed for ${type}, using fallback: ${error.message}`
        );
        return {};
      }
    };
    ...
    const findYoutubeVideoUrl = async (searchQuery, creator = "") => {
      try {
        ...
        const fullQuery = creator
          ? `${searchQuery} ${creator} ${examType} exam`
          : `${searchQuery} ${examType} exam`;

        const apiUrl = `https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&maxResults=10&order=viewCount&q=${encodeURIComponent(
          fullQuery
        )}&key=${process.env.YOUTUBE_API_KEY}`;

        console.log(
          `🎬 Searching YouTube for ${examType} exam: "${fullQuery}"`
        );
        const res = await fetch(apiUrl);

        if (!res.ok) {
          console.log(
            `❌ YouTube API search failed (${res.status}): ${res.statusText}`
          );
          return null;
        }
        ...
        const statsUrl = `https://www.googleapis.com/youtube/v3/videos?part=statistics,status&id=${videoIds.join(
          ","
        )}&key=${process.env.YOUTUBE_API_KEY}`;
        ...
        try {
          ...
          if (statsRes.ok) {
              ...
              .map((item) => {
                ...
                return {
                  ...
                  viewCount: stats?.statistics?.viewCount
                    ? parseInt(stats.statistics.viewCount)
                    : 0,
                  isAvailable:
                    stats?.status?.privacyStatus === "public" &&
            ...
            if (videosWithStats.length > 0) {
              ...
              console.log(
                `✅ Found ${examType} exam YouTube video (${viewCountFormatted} views): ${topVideo.snippet.title}`
              );
              return url;
            }
          }
          ...
          console.log(
            "⚠️ Failed to fetch video statistics, using first available result"
          );
        }
        ...
        if (firstVideo?.id?.videoId) {
          ...
          console.log(
            `🎬 Using first ${examType} exam search result: ${firstVideo.snippet.title}`
          );
          return url;
        }
    ...
    const findRealUrlWithGoogle = async (searchQuery, type) => {
      try {
        ...
        console.error(
          `Google search failed for "${searchQuery}":`,
          error.message
        );
        return null;
      }
    ...
    const processResourceCategory = async (resources, type) => {
      ...
      console.log(
        `🔄 Processing ${resources.length} ${type} resources for ${examType} exam`
      );

      const optimizedQueries = await generateOptimizedSearchQueries(
        resources,
        type
      );
      ...
      for (let i = 0; i < resources.length; i++) {
        ...
        if (!resource.url || resource.url.trim() === "") {
          ...
          const searchQuery =
            optimizedQueries[String(i + 1)] ||
            generateFallbackSearchQuery(resource, type);
          ...
          if (realUrl && realUrl.startsWith("http")) {
            if (type === "videos") {
              ...
              console.log(
                `✅ Found real YouTube URL for ${examType} exam: ${realUrl}`
              );
            } else {
            ...
            switch (type) {
              ...
              case "videos":
                ...
                resource.searchQuery =
                  `site:youtube.com ${videoTitle} ${creatorName}`.trim();
                resource.url = `https://www.youtube.com/results?search_query=${encodedQuery}`;
                console.log(
                  `⚠️ Video not found, setting searchQuery: "${resource.searchQuery}"`
                );
                break;
              case "tools":
            ...
            if (type !== "videos") {
              console.log(
                `⚠️ Using ${examType} exam search page fallback for: "${title}"`
              );
            }
          }
        }
    ...
    if (parsed.resources) {
      ...
      if (
        ...
        console.log(
          "Competitive exam resources are already in categorized format"
        );
        formattedResources = {
          books: Array.isArray(parsed.resources.books)
            ? await processResourceCategory(parsed.resources.books, "books")
            : [],
          courses: Array.isArray(parsed.resources.courses)
            ? await processResourceCategory(parsed.resources.courses, "courses")
            : [],
          articles: Array.isArray(parsed.resources.articles)
            ? await processResourceCategory(
                parsed.resources.articles,
                "articles"
              )
            : [],
          videos: Array.isArray(parsed.resources.videos)
            ? await processResourceCategory(parsed.resources.videos, "videos")
            : [],
          tools: Array.isArray(parsed.resources.tools)
            ? await processResourceCategory(parsed.resources.tools, "tools")
            : [],
      ...
      // If resources is an array of typed resources, categorize them
        ...
        console.log(
          "Competitive exam resources are in array format, categorizing..."
        );
        for (const resource of parsed.resources) {
          ...
          const processedResource = await processResourceCategory(
            [resource],
            category
          );
    ...
    for (const resourceType of Object.keys(formattedResources)) {
      formattedResources[resourceType] = await processResourceCategory(
        formattedResources[resourceType],
        resourceType
      );
    }
    ...
    const result = {
      summary:
        parsed.summary ||
        `AI-generated content for ${subject} in ${examType} examination`,
    ...
    console.log(
      "Successfully processed competitive exam module summary with comprehensive resources"
    );
    return result;
  } catch (error) {
...
7. Detailed subsections for **EACH** topic in the module:
   - Each topic should be a separate subsection with its own structured explanation.
   - Title, description, summary, and key points.
   - Subsections must include **TEN PAGES** as follows:
     • Pages 1–8: Detailed Conceptual Explanations
        - Page 1: Introduction & Foundation (200–300 words)
        - Page 2: Basic Theory & Laws (200–300 words)
        - Page 3: Mathematical Framework (200–300 words)
        - Page 4: Circuit Behavior & Intuition (200–300 words)
        - Page 5: Real-World Implementation (200–300 words)
        - Page 6: Advanced Use Cases & Challenges (200–300 words)
        - Page 7: Cross-topic Integration (200–300 words)
        - Page 8: Common Pitfalls & Misconceptions (200–300 words)
     • Page 9: Important Formulas and How to Use Them
     • Page 10: Other Important Notes (tips, tricks, revisions, caution areas, etc.)
   - Each page should include:
     - A clear title
     - 200–300 words of content
     - A key takeaway sentence
   - Clearly identify if each subsection needs:
     • Visualization (type: flowchart, formula, etc.)
     • Code/Math simulation (type: circuit behavior, equation manipulation, etc.)
     • Interactivity (drag-drop, sliders, calculators, etc.)
   - Add a “practicalExample” for each subsection to show real-world usage.
...
{ full JSON schema as provided in the previous version }
...
`;
  ...
  try {
    ...
    const problematicChars = responseText.match(
      /[\x00-\x08\x0B\x0C\x0E-\x1F\x7F-\x9F]/g
    );
    if (problematicChars) {
      console.log(
        `⚠️ Found ${problematicChars.length} problematic control characters in response`
      );
    }
    ...
    if (badEscapes) {
      console.log(
        `⚠️ Found ${badEscapes.length} potentially malformed escape sequences`
      );
    }
    ...
    if (!parsed) {
      ...
      try {
        ...
        console.log(
          "Regex extraction also failed, creating minimal fallback structure..."
        );

        // Ultimate fallback - create a basic structure that will work
        parsed = {
          summary:
            "AI-generated content is being processed. Please try again in a moment.",
          objectives: [
          ...
          beautifulSummaryElements: {
            keyInsights: [
              "Important concepts will be available once processing completes",
            ],
            practicalApplications: [
              "Real-world applications will be identified",
            ],
            whyItMatters:
              "This topic provides foundational knowledge for the subject area.",
            careerRelevance:
              "Understanding these concepts enhances professional capabilities.",
            difficultyLevel: context.learnerLevel || "Intermediate",
            prerequisites: [],
            estimatedStudyTime: "2-3 hours",
          },
          detailedSubsections: [
            {
              ...
              pages: [
                {
                  ...
                  pageTitle: "Introduction & Foundation",
                  content:
                    "Fundamental concepts and principles will be explained in detail once the content is fully processed. This page will introduce the basic ideas and provide necessary background knowledge.",
                  codeExamples: [],
                  mathematicalContent: [],
                  keyTakeaway: "Understanding the foundational concepts",
                },
                {
                  ...
                  pageTitle: "Deep Dive & Analysis",
                  content:
                    "Technical details and mechanisms will be explored in depth once processing is complete. This page will provide detailed explanations of how things work.",
                  codeExamples: [],
                  mathematicalContent: [],
                ...
                },
                {
                  ...
                  pageTitle: "Applications & Implementation",
                  content:
                    "Real-world applications and practical implementations will be demonstrated once the content is ready. This page will show how to apply the concepts in practice.",
                  codeExamples: [],
                  mathematicalContent: [],
              ...
              commonPitfalls: [
                "Common issues will be identified",
    ...
    if (parsed && typeof parsed === "object") {
      ...
      parsed.objectives = Array.isArray(parsed.objectives)
        ? parsed.objectives
        : [];
      ...
      parsed.detailedSubsections = Array.isArray(parsed.detailedSubsections)
        ? parsed.detailedSubsections
        : [];
      ...
      console.log(
        "❌ Parsing failed completely, parsed result is null or invalid"
      );
    ...
    const generateOptimizedSearchQueries = async (resources, type) => {
      try {
        ...
        console.log(
          `🤖 Batching ${resourcesNeedingOptimization.length} ${type} resources for search optimization`
        );
...
${
  type === "books"
    ? '- "Python Crash Course Eric Matthes programming book"'
    : ""
}
...
`;
    ...
    const findYoutubeVideoUrl = async (searchQuery, creator = "") => {
      try {
        ...
        const statsUrl = `https://www.googleapis.com/youtube/v3/videos?part=snippet,statistics,status&id=${videoIds.join(
          ","
        )}&key=${apiKey}`;
        const statsRes = await fetch(statsUrl);
        ...
        if (!statsRes.ok) {
          ...
          const validVideos = statsData.items
            ...
            .map((video) => ({
              ...
              relevanceTitleMatch: video.snippet.title
                .toLowerCase()
          ...
          if (ytResult) {
            ...
            if (typeof ytResult === "object" && ytResult.url) {
              console.log(
                `📊 Video details: ${
                  ytResult.channel
                } | ${ytResult.viewCount.toLocaleString()} views`
              );
              return ytResult.url;
            }
            return ytResult;
    ...
    if (responseText && responseText.length > 0) {
      ...
      console.error(
        "No response text available (likely due to quota or network error)"
      );
    }

    // Use robust regex extraction fallback
    console.log(
      "Falling back to regex extraction due to JSON parsing failure or missing response."
    );
    return extractDataWithRegex(responseText || "");
  }
}
