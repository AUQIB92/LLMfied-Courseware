...
export default function TestSeriesCreator({ onTestSeriesCreated }) {
  ...
  const parseMarkdownFile = (content) => {
    ...
    for (const line of lines) {
      ...
      const moduleMatch =
        trimmed.match(/^## (.+?) \((\d+)\s*(marks?|%|percent)?\)/i) ||
        trimmed.match(/^## (.+?)$/i);
      ...
      if (subtopicMatch && currentModule) {
        ...
        if (subtopic && !currentSubtopics.includes(subtopic)) {
          ...
          if (
            currentSubtopicGroup &&
            !subtopic.includes(currentSubtopicGroup)
          ) {
      ...
      if (
        ...
        trimmed.toLowerCase().includes("target audience:") ||
        trimmed.toLowerCase().includes("for:")
      ) {
        targetAudience = trimmed.replace(/^[^:]*:\s*/, "").trim();
      } else if (
        trimmed.toLowerCase().includes("prerequisites:") ||
    ...
    const totalMarks = topics.reduce(
      (sum, topic) => sum + (topic.marks || 0),
      0
    );
    ...
    if (totalMarks > 0) {
      ...
      topics.forEach((topic) => {
        topic.weightage = parseFloat(
          ((topic.marks / totalMarks) * 100).toFixed(2)
        );
      });
    } else {
      // Equal distribution if no marks specified
      const equalWeight =
        topics.length > 0 ? parseFloat((100 / topics.length).toFixed(2)) : 0;
      topics.forEach((topic, index) => {
        if (index === topics.length - 1) {
          // Last topic gets the remainder to ensure total is 100%
          const otherTotal = topics
            .slice(0, -1)
            .reduce((sum, t) => sum + equalWeight, 0);
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      if (parsedData.topics.length === 0) {
        setError(
          "No topics found in the markdown file. Please ensure your file has proper headings (# or ##)"
        );
        return;
      }
      ...
      setTestSeriesData((prev) => ({
        ...
        targetAudience:
          parsedData.basicInfo.targetAudience || prev.targetAudience,
        prerequisites: parsedData.basicInfo.prerequisites || prev.prerequisites,
      }));

      setSuccess(
        `Successfully parsed ${parsedData.topics.length} topics and auto-populated basic information from ${file.name}`
      );
      ...
      setError(
        "Failed to parse the markdown file. Please check the file format."
      );
    } finally {
...
`;
  ...
  const addTopic = () => {
    setTestSeriesData((prev) => {
      ...
      if (newTopics.length === 1) {
        ...
        const totalExceptLast = newTopics
          .slice(0, -1)
          .reduce((sum, topic) => sum + (parseFloat(topic.weightage) || 0), 0);
        const lastTopicWeightage = Math.max(
          0,
          parseFloat((100 - totalExceptLast).toFixed(2))
        );
  ...
  const updateTopic = (index, field, value) => {
    setTestSeriesData((prev) => {
      const updatedTopics = prev.topics.map((topic, i) =>
      ...
      if (field === "weightage" && updatedTopics.length >= 2) {
        ...
        if (index !== lastTopicIndex) {
          ...
          const totalExceptLast = updatedTopics
            .slice(0, -1)
            .reduce(
              (sum, topic) => sum + (parseFloat(topic.weightage) || 0),
              0
            );
  ...
  const normalizeWeightages = (topics) => {
    ...
    const otherTopicsTotal = normalizedTopics
      .slice(0, -1)
  ...
  const generateTestSeries = async () => {
    ...
    try {
      ...
      if (!testSeriesData.topics || testSeriesData.topics.length === 0) {
        throw new Error("Please add at least one topic");
      }

      // Validate that topics have names
      const invalidTopics = testSeriesData.topics.filter(
        (topic) => !topic.name?.trim()
      );
      if (invalidTopics.length > 0) {
        throw new Error("All topics must have a name");
      }

      // Validate that topics have subtopics
      const topicsWithoutSubtopics = testSeriesData.topics.filter(
        (topic) =>
          !topic.subtopics ||
          topic.subtopics.length === 0 ||
          topic.subtopics.every((sub) => !sub?.trim())
      );
      if (topicsWithoutSubtopics.length > 0) {
        throw new Error("All topics must have at least one subtopic");
      }
      ...
      const debugTotal = normalizedTopics.reduce(
        (sum, topic) => sum + (parseFloat(topic.weightage) || 0),
        0
      );
      ...
      const finalTopics = normalizedTopics.map((topic, index) => {
        if (index === normalizedTopics.length - 1) {
          ...
          const otherTotal = normalizedTopics
            .slice(0, -1)
            .reduce(
              (sum, t) => sum + Math.round(parseFloat(t.weightage) || 0),
              0
            );
          return {
            ...topic,
      ...
      const finalTotal = finalTopics.reduce(
      ...
      if (data.success) {
        setSuccess(
          "Test series created as draft successfully! You can now preview and publish it."
        );
        setGenerationProgress(100);
        setCurrentGenerationStep("Test series created as draft successfully!");

        // Store the created test series data
        setTestSeriesData((prev) => ({
          ...prev,
          _id: data.testSeries._id,
          status: "draft",
        }));
      ...
      let errorMessage = error.message || "Failed to generate test series";
      ...
      if (
        ...
        errorMessage +=
          "\n\n🔧 Troubleshooting steps:\n" +
          "1. The Perplexity AI service may be temporarily unavailable\n" +
          "2. Try again in a few minutes\n" +
          "3. Check Perplexity AI status page for outages";
      } else if (error.message?.includes("timed out")) {
        errorMessage +=
          "\n\n🔧 Troubleshooting steps:\n" +
          "1. Check your internet connection speed\n" +
          "2. Try generating fewer questions at once\n" +
          "3. Retry with simpler topics";
      }
  ...
  const handlePublishTestSeries = async () => {
    if (!testSeriesData._id) {
      setError("Please generate the test series first before publishing");
      return;
    }

    try {
      const response = await fetch(
        `/api/test-series/${testSeriesData._id}/status`,
        {
          method: "PATCH",
          headers: {
            ...getAuthHeaders(),
      ...
      if (response.ok) {
        setSuccess("Test series published successfully!");
        setTestSeriesData((prev) => ({
          ...prev,
          status: "published",
        }));
        ...
        const errorData = await response.json();
        setError(errorData.error || "Failed to publish test series");
      }
    } catch (error) {
      console.error("Error publishing test series:", error);
      setError("Failed to publish test series");
    }
  };

  const handlePreviewTestSeries = () => {
    if (!testSeriesData._id) {
      setError("Please generate the test series first before previewing");
      return;
    }

    // For now, show an alert with preview information
    // In the future, this could open a preview modal or navigate to a preview page
    alert(
      `Preview Test Series: ${testSeriesData.title}\n\nStatus: ${testSeriesData.status}\nTotal Tests: ${testSeriesData.totalTests}\nQuestions per Test: ${testSeriesData.questionsPerTest}\n\nPreview functionality will be implemented in the test series management dashboard.`
    );
  };

  const totalWeightage = parseFloat(
    testSeriesData.topics
      .reduce((sum, topic) => sum + (parseFloat(topic.weightage) || 0), 0)
      .toFixed(2)
  );
  const totalQuestions =
    testSeriesData.totalTests * testSeriesData.questionsPerTest;
  const numericalCount = Math.round(
    (totalQuestions * testSeriesData.numericalPercentage) / 100
  );
    ...
    <div className="space-y-6">
      <Card className="border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50">
        <CardHeader>
          ...
          <CardDescription>
            Design and generate comprehensive test series with AI-powered
            question creation using Perplexity AI
          </CardDescription>
        </CardHeader>
      </Card>
      ...
      <Tabs value={activeStep} onValueChange={setActiveStep}>
        ...
        <TabsContent value="basic" className="space-y-6">
          ...
          {testSeriesData._id && (
            <Alert
              className={
                testSeriesData.status === "published"
            ...
            >
              ...
              >
                <strong>Test Series Created:</strong> {testSeriesData.title} -
                Status: {testSeriesData.status}
                {testSeriesData.status === "draft" && (
                  <span className="ml-2">
          ...
          {uploadedFileName && !testSeriesData._id && (
            <Alert className="border-blue-200 bg-blue-50">
              <CheckCircle className="h-4 w-4 text-blue-600" />
              <AlertDescription className="text-blue-800">
                <strong>Auto-populated from {uploadedFileName}:</strong> Review
                and modify the information below as needed.
                {testSeriesData.subject && (
                  <span className="ml-2">
                    Subject detected: <strong>{testSeriesData.subject}</strong>
                  </span>
                )}
              </AlertDescription>
            </Alert>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Enter the basic details for your test series
              </CardDescription>
            </CardHeader>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setTestSeriesData((prev) => ({ ...prev, subject: value }))
                    }
                  >
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="Nuclear Engineering">
                        Nuclear Engineering
                      </SelectItem>
                      ...
                      <SelectItem value="Data Science & Engineering">
                      ...
                      <SelectItem value="Strategic Management">
                      ...
                      <SelectItem value="International Business">
                      ...
                      {/* Law & Legal Studies */}
                      <SelectItem value="Constitutional Law">
                        Constitutional Law
                      </SelectItem>
                      ...
                      <SelectItem value="International Law">
                        International Law
                      </SelectItem>
                      ...
                      <SelectItem value="Landscape Architecture">
                      ...
                      <SelectItem value="Veterinary Science">
                        Veterinary Science
                      </SelectItem>
                      <SelectItem value="Fisheries Science">
                        Fisheries Science
                      </SelectItem>
                      ...
                      <SelectItem value="Secondary Education">
                        Secondary Education
                      </SelectItem>
                      ...
                      <SelectItem value="Sports Science">
                        Sports Science
                      </SelectItem>
                      <SelectItem value="Sports Management">
                        Sports Management
                      </SelectItem>
                      ...
                      <SelectItem value="Data Interpretation">
                        Data Interpretation
                      </SelectItem>
                      ...
                      <SelectItem value="Reading Comprehension">
                        Reading Comprehension
                      </SelectItem>
                      <SelectItem value="General Awareness">
                        General Awareness
                      </SelectItem>
                      ...
                      <SelectItem value="Tourism & Travel">
                        Tourism & Travel
                      </SelectItem>
                      ...
                      <SelectItem value="Maritime Studies">
                        Maritime Studies
                      </SelectItem>
                      <SelectItem value="Library Science">
                        Library Science
                      </SelectItem>
                      <SelectItem value="Social Work">Social Work</SelectItem>
                      <SelectItem value="Retail Management">
                        Retail Management
                      </SelectItem>
                      <SelectItem value="Fashion Technology">
                        Fashion Technology
                      </SelectItem>
                      <SelectItem value="Textile Technology">
                        Textile Technology
                      </SelectItem>
                    </SelectContent>
                  </Select>
        ...
        <TabsContent value="syllabus" className="space-y-6">
          ...
          <Card className="border-green-200 bg-gradient-to-r from-green-50 to-emerald-50">
            <CardHeader>
              ...
              <CardDescription>
                Upload a .md file to automatically parse topics, subtopics,
                marks distribution, and basic information
              </CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    <div className="flex items-center gap-2 p-4 border-2 border-dashed border-green-300 rounded-lg hover:border-green-400 hover:bg-green-50/50 transition-colors">
                      ...
                      <span className="text-green-700">
                        {isParsingFile
                          ? "Parsing file..."
                          : uploadedFileName ||
                            "Click to upload .md file or drag & drop"}
                      </span>
                      {isParsingFile && (
              ...
              <div className="mt-4 text-sm text-green-600">
                ...
                <ul className="list-disc list-inside space-y-1 mt-2">
                  <li>
                    <strong>Title:</strong> First # heading becomes the test
                    series title
                  </li>
                  <li>
                    <strong>Topics:</strong> Use "## Topic Name (X Marks)"
                    format
                  </li>
                  <li>
                    <strong>Subtopic Groups:</strong> Use "### Group Name" for
                    subtopic categories
                  </li>
                  <li>
                    <strong>Subtopics:</strong> Use bullet points (-,*,+) under
                    ### groups
                  </li>
                  <li>
                    <strong>Marks:</strong> Include in parentheses after topic
                    name (e.g., "## Algebra (25 Marks)")
                  </li>
                  <li>
                    <strong>Description:</strong> Add "Description: your text"
                  </li>
                  <li>
                    <strong>Target Audience:</strong> Add "Target Audience: your
                    text"
                  </li>
                  <li>
            ...
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {totalWeightage !== 100 &&
                    testSeriesData.topics.length > 1 && (
                      <Button
                        onClick={() => {
                          const normalizedTopics = normalizeWeightages(
                            testSeriesData.topics
                          );
                          setTestSeriesData((prev) => ({
                            ...prev,
              ...
              <CardDescription>
                Define topics with their weightage and subtopics for
                comprehensive coverage. The last topic's weightage is
                automatically calculated to ensure the total equals 100%.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
                    ...
                    <div className="flex items-start gap-4 mb-4">
                      <div className="flex-1">
                        ...
                        <Input
                          value={topic.name}
                          onChange={(e) =>
                            updateTopic(topicIndex, "name", e.target.value)
                          }
                      ...
                      <div className="w-32">
                        <Label>
                          Weightage (%)
                          {topicIndex === testSeriesData.topics.length - 1 &&
                            testSeriesData.topics.length > 1 && (
                        ...
                        <Input
                          ...
                          placeholder={
                            testSeriesData.topics.length === 1 ? "100" : "25.5"
                          }
                          disabled={
                            topicIndex === testSeriesData.topics.length - 1 &&
                            testSeriesData.topics.length > 1
        ...
        <TabsContent value="config" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Configure test parameters and marking scheme
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setTestSeriesData((prev) => ({
                        ...prev,
                        totalTests: parseInt(e.target.value) || 1,
                      }))
                    }
                    min="1"
                    max="50"
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setTestSeriesData((prev) => ({
                        ...prev,
                        questionsPerTest: parseInt(e.target.value) || 1,
                      }))
                    }
                    min="10"
                    max="200"
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setTestSeriesData((prev) => ({
                        ...prev,
                        timePerTest: parseInt(e.target.value) || 60,
                      }))
                    }
                    min="30"
                    max="300"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="numericalPercentage">
                    Numerical Questions (%)
                  </Label>
                  <Input
                ...
                <div>
                  <Label htmlFor="theoreticalPercentage">
        ...
        <TabsContent value="generate" className="space-y-6">
          <Card className="border-indigo-200 bg-gradient-to-r from-indigo-50 to-purple-50">
            <CardHeader>
              ...
              <CardDescription>
                Advanced AI research and question generation with extensive
                validation and market-leading features. Test series will be
                created as draft for preview before publishing.
              </CardDescription>
            </CardHeader>
            <CardContent>
              ...
              {!isGenerating && !testSeriesData._id && (
                  ...
                  <div className="text-center space-y-4">
                    <div className="p-4 bg-green-50 rounded-lg border border-green-200">
                      ...
                      <p className="text-sm text-green-600 mt-1">
                        Status:{" "}
                        {testSeriesData.status === "draft"
                          ? "Draft (Ready for Preview & Publishing)"
                          : "Published"}
                      </p>
                    </div>
                    ...
                    <div className="flex gap-3 justify-center">
                      <Button
                        variant="outline"
                        onClick={handlePreviewTestSeries}
                      >
      ...
      <div className="flex items-center justify-between pt-6 border-t">
        <div className="flex gap-2">
          ...
          {testSeriesData._id && (
            <>
              <Button variant="outline" onClick={handlePreviewTestSeries}>
                <Eye className="w-4 h-4 mr-2" />
                Preview Test Series
              </Button>
              ...
              {testSeriesData.status === "published" && (
                <Badge
                  variant="default"
                  className="bg-green-100 text-green-800 px-3 py-2"
                >
                  <CheckCircle className="w-4 h-4 mr-2" />
                  Published
                </Badge>
        ...
        <div className="flex gap-2">
          {activeStep !== "generate" && !testSeriesData._id && (
            <Button
              onClick={() => setActiveStep("generate")}
              disabled={
                totalWeightage !== 100 || testSeriesData.topics.length === 0
              }
            >
          ...
          {testSeriesData._id && testSeriesData.status === "published" && (
            <Button
              variant="outline"
              onClick={() => {
                // Reset the form to create a new test series
                setTestSeriesData({
                  title: "",
                  description: "",
                  subject: "",
