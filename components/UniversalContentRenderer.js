...
const comprehensiveLaTeXSanitization = (content) => {
  ...
  try {
    ...
    sanitized = sanitized
      ...
      .replace(
        /\\\\(frac|sqrt|sum|int|lim|prod|alpha|beta|gamma|delta|epsilon|theta|lambda|mu|pi|sigma|phi|omega|infty|partial|nabla)/g,
        "\\$1"
      )
    ...
    if (dollarMatches && dollarMatches.length % 2 !== 0) {
      ...
      sanitized =
        sanitized.substring(0, lastDollar) +
        sanitized.substring(lastDollar + 1);
    }
...
const getMarkdownComponents = (inline = false, onError = null) => {
  ...
  if (inline) {
    return {
      ...
      table: ({ children }) => (
        <span className="mx-2">[Table: {children}]</span>
      ),
    ...
    return {
      ...
      p: ({ children, ...props }) => {
        ...
        if (hasBlockElements) {
          ...
          return (
            <div
              className="mb-4 leading-relaxed text-gray-800 last:mb-0"
              {...props}
            >
              {children}
            </div>
      ...
      h1: ({ children, ...props }) => (
        <h1
          className="text-3xl font-bold mb-6 mt-8 text-gray-900 first:mt-0"
          {...props}
        >
          {children}
        </h1>
      ...
      h2: ({ children, ...props }) => (
        <h2
          className="text-2xl font-bold mb-5 mt-7 text-gray-900 first:mt-0"
          {...props}
        >
          {children}
        </h2>
      ...
      h3: ({ children, ...props }) => (
        <h3
          className="text-xl font-semibold mb-4 mt-6 text-gray-900 first:mt-0"
          {...props}
        >
          {children}
        </h3>
      ...
      h4: ({ children, ...props }) => (
        <h4
          className="text-lg font-semibold mb-3 mt-5 text-gray-900 first:mt-0"
          {...props}
        >
          {children}
        </h4>
      ...
      h5: ({ children, ...props }) => (
        <h5
          className="text-base font-medium mb-2 mt-4 text-gray-900 first:mt-0"
          {...props}
        >
          {children}
        </h5>
      ...
      h6: ({ children, ...props }) => (
        <h6
          className="text-sm font-medium mb-2 mt-3 text-gray-800 first:mt-0"
          {...props}
        >
          {children}
        </h6>
      ...
      blockquote: ({ children, ...props }) => (
        <blockquote
          className="border-l-4 border-blue-500 pl-4 my-4 italic text-gray-700 bg-blue-50 py-2 rounded-r"
          {...props}
        >
          {children}
        </blockquote>
      ...
      pre: ({ children, ...props }) => (
        <pre
          className="bg-gray-50 rounded-lg p-4 overflow-x-auto my-4 border"
          {...props}
        >
          {children}
        </pre>
      ...
      div: ({ className, children, ...props }) => {
        if (className && className.includes("katex-display")) {
          return (
            <div
              className={`${className} my-6 text-center overflow-x-auto`}
              {...props}
            >
              {children}
            </div>
...
const UniversalContentRenderer = ({
  ...
  const renderContent = () => {
    ...
    const remarkPlugins =
      renderStrategy === "plaintext" ? [] : [remarkMath, remarkGfm];
    const rehypePlugins =
      renderStrategy === "plaintext"
        ? []
        : [
    ...
    return (
      <WrapperComponent
        className={`universal-content-renderer ${renderStrategy} ${
          inline ? "inline" : "block"
        } ${className}`}
      ...
      >
        ...
        {process.env.NODE_ENV === "development" && enableAnalytics && (
          <div className="text-xs text-gray-400 mt-2 p-1 bg-gray-50 rounded hidden">
            ...
            Complexity: {contentAnalysis.complexity} | Math:{" "}
            {contentAnalysis.mathCount} | Markdown:{" "}
            {contentAnalysis.markdownCount}
          </div>
        )}
      </WrapperComponent>
    );
