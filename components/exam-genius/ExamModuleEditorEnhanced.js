...
export default function ExamModuleEditorEnhanced({
  ...
  const detailedSubsections = useMemo(() => {
    ...
    allHeadings.forEach((h) => {
      // Assuming sections are level 3 headings like ### 1.1 Title
      if (h.level === 3) {
        sections[h.number] = h.name;
      }
    });

    const subsectionsFromMarkdown = allHeadings;
  ...
  const generateSubsectionQuiz = async (
    ...
    if (!subsectionContent || subsectionContent.trim().length < 10) {
      toast.error(
        "❌ Subsection content is required and must be at least 10 characters long"
      );
            ...
            <CardContent className="space-y-6">
              {detailedSubsections.length === 0 ? (
                ...
                currentPageExplanations.map((subsection, pageIndex) => {
                  ...
                  const currentExplanationContent =
                    explanationPages[currentSubsectionPage]?.content ||
                      ...
                      <CardContent className="space-y-4">
                        ...
                        <div className="space-y-3">
                          ...
                          {editingSubsection === globalIndex ? (
                            <Textarea
                              value={
                                explanationPages[currentSubsectionPage]
                                  ?.content || ""
                              }
                              onChange={(e) => {
                            ...
                            >
                              ...
                              {currentExplanationContent &&
                              currentExplanationContent.trim() ? (
                                <div className="prose prose-sm max-w-none">
                                  <MathMarkdownRenderer
                                    content={currentExplanationContent}
                                  />
                                </div>
                              ) : (
                                <div className="space-y-2">
                                  ...
                                  {process.env.NODE_ENV === "development" && (
                                    <div className="text-xs text-gray-400 border-t pt-2">
                                      <strong>Debug:</strong> page content is
                                      empty.
                                    </div>
                                  )}
                                </div>
