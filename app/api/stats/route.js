...
export async function GET(request) {
  try {
    ...
    if (user.role === "educator" || type === "educator") {
      ...
      if (excludeExamGenius) {
        courseFilter.$and = [
          {
            $or: [
              { isExamGenius: { $ne: true } },
              { isExamGenius: { $exists: false } },
            ],
          },
          {
            $or: [
              { isCompetitiveExam: { $ne: true } },
              { isCompetitiveExam: { $exists: false } },
            ],
          },
        ];
      }
      ...
      const recentEnrollments = await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(
            () => reject(new Error("Recent enrollments timeout")),
            5000
          )
        ),
      ]);
      ...
      const weeklyEnrollments = await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(
            () => reject(new Error("Weekly enrollments timeout")),
            5000
          )
        ),
      ]);
      ...
      const coursePerformance = await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(
            () => reject(new Error("Course performance timeout")),
            5000
          )
        ),
      ]);
      ...
      const topCourses = coursePerformance
        ...
        .map((perf) => {
          const course = courses.find(
            (c) => c._id && perf._id && c._id.toString() === perf._id.toString()
          );
          return {
            _id: perf._id,
            ...
            completionRate:
              perf.totalEnrollments > 0
                ? Math.round(
                    (perf.completedCount / perf.totalEnrollments) * 100
                  )
                : 0,
      ...
      const responseData = {
        ...
        engagementMetrics: {
          ...
          averageCoursesPerStudent:
            totalEnrollments > 0
              ? Math.round((totalEnrollments / activeLearners) * 10) / 10
              : 0,
        ...
        courses: courses.map((course) => {
          const perf = coursePerformance.find(
            (p) =>
              p._id && course._id && p._id.toString() === course._id.toString()
          );
          return {
            _id: course._id,
            title: course.title,
      ...
      const enrollments = await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(
            () => reject(new Error("Learner enrollments timeout")),
            5000
          )
        ),
      ]);
      ...
      const enrolledCourses =
        enrolledCourseIds.length > 0
          ? await Promise.race([
              ...
              new Promise((_, reject) =>
                setTimeout(
                  () => reject(new Error("Enrolled courses timeout")),
                  5000
                )
              ),
            ])
      ...
      const learningStreak =
        recentActivity.length > 0 ? Math.min(recentActivity.length, 7) : 0;
      ...
      const learnerStats = {
        ...
        lastActiveDate: (() => {
          ...
          return validActivities.length > 0
            ? Math.max(
                ...validActivities.map((a) => new Date(a.lastAccessedAt))
              )
            : null;
        })(),
      };
    ...
    if (error.message.includes("timeout")) {
      ...
      errorMessage =
        "Network connection timeout. Please check your connection and try again.";
      statusCode = 503;
